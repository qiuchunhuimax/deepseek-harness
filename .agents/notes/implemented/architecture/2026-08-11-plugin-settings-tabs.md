...
The section owner contributes a `configurable` tab that declares the existing nested `settings.plugin.item` list. Configuration cards keep their namespace bindings, draft state, validation, and writes unchanged. `@deepseek-ai/dsh-client-ui-settings-plugin-inventory` contributes `custom` and `all` tabs to `settings.plugins.tab`; both read the same unchanged Host Loader observer, generated Remote namespace, and DTO. The custom projection includes every module name outside the official `@deepseek-ai/` catalog (community, GitHub-installed, and locally authored packages alike, plus one exception: `cordis:include`, the unscoped framework primitive `@deepseek-ai/dsh-base` inserts under its own Cordis name), while the all projection preserves the complete inventory and both retain local search. Disabled inventory entries omit the redundant unmounted runtime state from summaries and details, while enabled entries continue to expose their Cordis phase.

The first ordered tab is selected by default. A tab mounts only when first selected and then remains mounted but hidden while the Plugins section stays mounted. This delays each inventory RPC until the user opens **Custom plugins** or **Plugin list** and preserves drafts, per-tab search text, disclosure state, and fetched snapshots while switching tabs. Closing Settings unmounts the section, so reopening it obtains a fresh inventory snapshot when an inventory tab is selected again.
...
**Hard-code tab labels and components in the section owner.** Rejected because a new feature would require editing the owner, and HMR teardown could leave chrome for a contribution that no longer exists. The slot ledger already provides identity, ordering, localization, and cascade semantics.

**Infer custom/official status from install paths or Git specifications.** Rejected because the Loader inventory does not own package-manager provenance, and adding it to the Host DTO would couple a read-only runtime projection to one installation mechanism. The official `@deepseek-ai/` namespace is stable at the module identity already exposed by the Loader; everything outside it (regardless of how it was installed) is custom.
...
Settings has one Plugins navigation row, ordered before Agent Presets, with **Plugin configuration**, **Custom plugins**, and **Plugin list** tabs. Agent Presets remains an independent section because it edits per-session agent compositions rather than the live Host Loader tree.
