...
- **Namespace-based classification** — the custom view treats anything outside the `@deepseek-ai/` catalog (community, GitHub-installed, and locally authored packages alike) as custom; it does not infer ownership from filesystem or Git installation details, so it cannot distinguish a third-party plugin from one the deployment operator wrote themselves.
